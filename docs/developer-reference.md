...
All rules — including custom rules — are automatically protected on non-interactive surfaces (WP-CLI, Cron, XML-RPC, REST/Application Passwords, and potentially WPGraphQL) via the configurable policy settings, even if they don't define AJAX or REST criteria.
