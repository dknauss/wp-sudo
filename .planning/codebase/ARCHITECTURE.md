...
- User meta keys (Sudo_Session):
  ...
  - `_wp_sudo_failed_attempts`: Legacy scalar count (backwards compatibility)
  - `_wp_sudo_failure_event`: Append-only rows for failure event tracking
  - `_wp_sudo_throttle_until`: Non-blocking throttle window timestamp
